...
def is_inside_field(row: int, col: int, grid: List[List[Union[int, str]]]) -> bool:
    """
    Checks if the position is inside the field.

    Args:
        row: the row index
        col: the column index
        grid: the matrix representing the field
...
def has_enough_tea(current_tea: int, required_tea: int) -> bool:
    """
    Checks if Alice collected enough tea bags.

    Args:
        current_tea: tea bags collected so far
        required_tea: number of tea bags needed

    Returns:
        True if enough tea bags collected, otherwise False
    """
